...
export default function SalesmanOrderPlacement() {
  ...
  const applyDiscount = (productId, discountPercentage) => {
    ...
    if (item) {
      const discountedPrice = item.originalPrice * (1 - discountPercentage / 100);
      updateCustomPrice(productId, discountedPrice.toFixed(2));
    }
  };
    ...
    const orderData = {
      items: cart.map(item => {
        ...
        return {
          ...
          discountPercentage: ((item.originalPrice - effectivePrice) / item.originalPrice * 100).toFixed(2)
        };
      }),
  ...
  const printReceipt = async () => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Order Receipt - ${lastOrder.shopkeeper?.shopName || lastOrder.shopkeeper?.name || 'Shop'}</title>
          <style>
        ...
        <body>
          <div class="header">
            ...
            <p>Order Receipt</p>
            <div class="shop-name">For: ${lastOrder.shopkeeper?.shopName || lastOrder.shopkeeper?.name || 'Shop'}</div>
          </div>
          ${receiptContent}
        </body>
      </html>
...
${lastOrder.items.map(item => 
  `• ${item.product?.name || 'Product'}: ${item.quantity} x PKR${item.unitPrice?.toFixed(2) || '0.00'} = PKR${item.totalPrice?.toFixed(2) || '0.00'}`
).join('\n')}
...
Thank you for your business with Ideal Nimko! 🎉`;
  ...
  const shareViaEmail = () => {
    ...
    const body = `Dear ${lastOrder.shopkeeper?.name || 'Valued Customer'},
...
${lastOrder.notes ? `NOTES: ${lastOrder.notes}` : ''}
...
Ideal Nimko Ltd.`;
    ...
    const emailUrl = `mailto:${lastOrder.shopkeeper?.email || ''}?subject=${encodedSubject}&body=${encodedBody}`;
            ...
            <div className="bg-white rounded-lg p-3 sm:p-6 max-w-2xl w-full max-h-[95vh] overflow-y-auto">
              ...
              <div id="receipt-content">
                ...
                <div className="order-info">
                  <p><strong>Order ID:</strong> {lastOrder._id}</p>
                  <p><strong>Order Date:</strong> {new Date(lastOrder.createdAt).toLocaleString()}</p>
                  <p><strong>Shop Name:</strong> {lastOrder.shopkeeper?.shopName || lastOrder.shopkeeper?.name || 'N/A'}</p>
                  <p><strong>Shopkeeper:</strong> {lastOrder.shopkeeper?.name}</p>
                  <p><strong>Salesman:</strong> {lastOrder.placedBySalesman?.name || lastOrder.salesman?.name}</p>
                  <p><strong>Delivery Address:</strong> {lastOrder.deliveryAddress}</p>
                  <p><strong>Payment Method:</strong> {lastOrder.paymentMethod}</p>
                  <p><strong>Payment Status:</strong> 
                    <span className={`ml-2 px-2 py-1 rounded text-xs font-medium ${
                      lastOrder.paymentStatus === 'paid' 
                        ? 'bg-green-100 text-green-800' 
                        : lastOrder.paymentStatus === 'partial'
                  ...
                  {lastOrder.amountPaid > 0 && (
                    <p><strong>Amount Paid:</strong> <span className="text-green-600">PKR {lastOrder.amountPaid?.toFixed(2) || '0.00'}</span></p>
                  )}
                  {lastOrder.pendingAmount > 0 && (
                    <p><strong>Order Pending Amount:</strong> <span className="text-orange-600">PKR {lastOrder.pendingAmount?.toFixed(2) || '0.00'}</span></p>
                  )}
                  {lastOrder.shopkeeper?.pendingAmount > 0 && (
                    <p><strong>Shopkeeper Total Pending Amount:</strong> <span className="text-red-600 font-semibold">PKR {lastOrder.shopkeeper?.pendingAmount?.toFixed(2) || '0.00'}</span></p>
                  )}
                ...
                <table className="items-table">
                  ...
                  <tfoot>
                    <tr>
                      ...
                      <td>PKR {lastOrder.totalAmount?.toFixed(2) || '0.00'}</td>
                    </tr>
                    {lastOrder.amountPaid > 0 && (
                      <tr>
                        <td colSpan="3">Amount Paid:</td>
                        <td className="text-green-600">PKR {lastOrder.amountPaid?.toFixed(2) || '0.00'}</td>
                      </tr>
                    )}
                    {lastOrder.pendingAmount > 0 && (
                      <tr>
                        <td colSpan="3">Pending from this order:</td>
                        <td className="text-orange-600">PKR {lastOrder.pendingAmount?.toFixed(2) || '0.00'}</td>
                      </tr>
                    )}
                  </tfoot>
                </table>
              ...
              {selectedShopkeeperDetails?.pendingAmount && selectedShopkeeperDetails.pendingAmount > 0 && (
                <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                  <p className="text-sm font-medium text-red-800">
                    Current Pending Amount: PKR {selectedShopkeeperDetails.pendingAmount.toFixed(2)}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="bg-white rounded-lg shadow p-4 sm:p-6 sticky top-4">
              ...
              {cart.length === 0 ? (
                ...
                <div className="space-y-4">
                  {cart.map((item) => {
                    const discountPercentage = ((item.originalPrice - item.customPrice) / item.originalPrice * 100).toFixed(1);
                    const isDiscounted = item.customPrice < item.originalPrice;
                    
                    return (
                      <div key={item.productId} className="border border-gray-200 rounded-lg p-4 space-y-3">
                        <div className="flex items-center space-x-3">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center space-x-2">
                              <span className="text-sm text-gray-500">Original: PKR {item.originalPrice}</span>
                              {isDiscounted && (
  ...
  <label className="block text-xs font-medium text-gray-700 mb-1">
   ...
   <input
     ...
     onChange={(e) => {
       ...
       // Allow empty input for clearing, but clean leading zeros when typing
       if (value === '') {
   ...
   />
                          ...
                          <div>
                            ...
                            <div className="flex space-x-1">
                              <button
                                onClick={() => applyDiscount(item.productId, 5)}
                                className="px-2 py-1 text-xs bg-blue-100 text-blue-800 rounded hover:bg-blue-200"
                              >
                                5%
                              </button>
                              <button
                                onClick={() => applyDiscount(item.productId, 10)}
                                className="px-2 py-1 text-xs bg-blue-100 text-blue-800 rounded hover:bg-blue-200"
                              >
                                10%
                              </button>
                              <button
                                onClick={() => applyDiscount(item.productId, 20)}
                                className="px-2 py-1 text-xs bg-blue-100 text-blue-800 rounded hover:bg-blue-200"
                              >
                                20%
                              </button>
                              <button
                                onClick={() => updateCustomPrice(item.productId, item.originalPrice)}
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-2">
                            ...
                            <button
                              ...
                              className="w-6 h-6 bg-gray-200 rounded-full flex items-center justify-center text-sm hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed"
                              title={item.quantity >= (item.stock || 0) ? `Max: ${item.stock || 0} units` : ''}
                            >
                              +
                            </button>
                          ...
                          <div className="text-right">
                            <p className="text-sm font-semibold text-gray-900">
                              PKR {((item.customPrice === '' ? item.originalPrice : item.customPrice) * item.quantity).toFixed(2)}
                            </p>
                            {isDiscounted && (
                              <p className="text-xs text-green-600">
                                Saved: PKR {((item.originalPrice - (item.customPrice === '' ? item.originalPrice : item.customPrice)) * item.quantity).toFixed(2)}
                              </p>
                            )}
                          ...
                          {hasDiscounts && (
                            <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                              <div className="flex justify-between text-sm">
                                ...
                                <span className="text-green-800">PKR {totalOriginalAmount.toFixed(2)}</span>
                              </div>
                              <div className="flex justify-between text-sm">
                                <span className="text-green-800">Total Savings:</span>
                                <span className="text-green-800 font-semibold">PKR {totalSavings.toFixed(2)}</span>
                              </div>
                            </div>
                          )}
                          <div className="flex justify-between text-lg font-semibold">
                            <span>Final Total:</span>
                            <span>PKR {totalCustomAmount.toFixed(2)}</span>
                          </div>
                        </>
                      );
                    })()}
              ...
              {cart.length > 0 && (
                <form onSubmit={handleSubmitOrder} className="mt-6 space-y-4">
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                    ...
                    <div className="space-y-1 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="text-red-600 font-semibold">PKR {(selectedShopkeeperDetails?.pendingAmount || 0).toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-700">Order Total:</span>
                        <span className="font-semibold">PKR {getTotalAmount().toFixed(2)}</span>
                      </div>
                      <div className="border-t border-blue-200 pt-1 mt-1">
                        <div className="flex justify-between">
                          <span className="text-gray-700">Amount to Pay Now:</span>
                          <span className="text-green-700 font-semibold">PKR {(parseFloat(orderForm.amountPaid) || 0).toFixed(2)}</span>
                        </div>
                      </div>
                      <div className="border-t border-blue-300 pt-2 mt-2">
                        <div className="flex justify-between">
                          ...
                          <span className="font-bold text-red-700">
                            PKR {((selectedShopkeeperDetails?.pendingAmount || 0) + getTotalAmount() - (parseFloat(orderForm.amountPaid) || 0)).toFixed(2)}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    {orderForm.amountPaid && parseFloat(orderForm.amountPaid) >= 0 && (
                      <div className="mt-2 text-sm">
                        {parseFloat(orderForm.amountPaid) > getTotalAmount() ? (
                          <>
                            <span className="text-green-600">Excess payment: </span>
                            <span className="font-semibold text-green-600">
                              PKR {(parseFloat(orderForm.amountPaid) - getTotalAmount()).toFixed(2)}
                            </span>
                            <p className="text-xs text-green-700 mt-1">
                              This will reduce the pending amount by PKR {(parseFloat(orderForm.amountPaid) - getTotalAmount()).toFixed(2)}
                            </p>
                          </>
                        ) : parseFloat(orderForm.amountPaid) < getTotalAmount() ? (
                          <>
                            ...
                            <span className="font-semibold text-orange-600">
                              PKR {(getTotalAmount() - parseFloat(orderForm.amountPaid)).toFixed(2)}
                            </span>
                          </>
                        ) : (
                  ...
                  <button
                    type="submit"
                    disabled={submitting || !selectedShopkeeper}
