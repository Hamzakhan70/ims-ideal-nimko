...
export default function UserManagement() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingUser && ! submitData.password) { // Don't send password if it's empty during update
            ...
            render: (user) => (
                <span className={
                    `inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                        user.role === 'superadmin' ? 'bg-purple-100 text-purple-800' : user.role === 'admin' ? 'bg-blue-100 text-blue-800' : user.role === 'salesman' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                    }`
                }>
                    {
                    user.role
        ...
        },
        {
            ...
            render: (user) => (
                <span className="text-sm text-gray-900">
                    {
                    user.role === 'shopkeeper' ? `PKR${
                        (user.pendingAmount || 0).toFixed(2)
                    }` : '-'
                } </span>
            )
        }, {
    ...
    return (
        <div className="p-6">
            ...
            <div className="bg-white rounded-lg p-4 mb-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <select value={selectedRoleFilter}
                            onChange={
                                (e) => setSelectedRoleFilter(e.target.value)
                            }
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <div>
                        ...
                        <select value={selectedCityFilter}
                            onChange={
                                (e) => setSelectedCityFilter(e.target.value)
                            }
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    ...
                                    <div>
                                        ...
                                        <input type="number"
                                            ...
                                            onChange={
                                                (e) => setFormData({
                                                    ...formData,
                                                    pendingAmount: parseFloat(e.target.value)
                                                })
                                            }
                                            min="0"
                                            step="0.01"
            ...
            <DataTable columns={columns}
                ...
                emptyMessage="No users found. Create your first user above."/> {/* Pagination */}
